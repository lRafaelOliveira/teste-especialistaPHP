...
class RouterBase
{
    ...
    protected function handleRequest($routes)
    {
        try {
            ...
            $url = Request::getUrl();
            if (preg_match('/\.(css|js|png|jpg|jpeg|gif|svg|woff|woff2|ttf|eot|ico)$/', $url)) {
                // Serve o arquivo diretamente da pasta public
                $filePath = __DIR__ . '/../public' . $url;
                $filePathStorage = __DIR__ . '/../storage' . $url;
                if (file_exists($filePath)) {
                    header('Content-Type: ' . mime_content_type($filePath));
                    readfile($filePath);
                    exit;
                } else if (file_exists($filePathStorage)) {
                    header('Content-Type: ' . mime_content_type($filePathStorage));
                    readfile($filePath);
                    exit;
                } else {
