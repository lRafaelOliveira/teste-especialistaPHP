...
class Request
{
    ...
    {
        // Exemplo: /admin/livros/5
        $url = $_SERVER['REQUEST_URI'] ?? '/';

        // Remove query string, se houver
        $url = strtok($url, '?');

        // Opcional: remover o BASE_DIR se não estiver rodando na raiz do domínio
        // Exemplo: seu app está em /meuprojeto/public, defina BASE_DIR = '/meuprojeto/public'
        if (defined('BASE_DIR') && BASE_DIR !== '' && strpos($url, BASE_DIR) === 0) {
            $url = substr($url, strlen(BASE_DIR));
        }

        // Sempre retorna com barra inicial
        return '/' . ltrim($url, '/');
    }
